...
export function buildSystemPrompt(): string {
  ...
  return [
    ...
    "- When the user names a discrete subject (a dress, a person, a car), call",
    "  segment with that referring expression first. Use the returned maskId on",
    "  set_operations via `mask`.",
    "- For tonal or positional areas — 'darken the bright top', 'the sky is blown",
    "  out', 'burn the corners' — call create_mask (luminance_range, linear_gradient,",
    "  or radial_gradient) instead of segment. Segmentation is a poor fit for",
    "  amorphous regions.",
    "- For 'make the grass pop', 'mute the oranges', 'colder blues': use hsl_mixer",
    "  on the matching hue band (no mask). Reach for a color_range mask only when a",
    "  non-color op must target a hue (sharpen just the greens, blur the reds).",
    ...
    "  highlights' (Lightroom-style dual recovery). Prefer blacks_whites when the",
    "  user wants the endpoints — crush/lift blacks, clip or pull in whites. Keep",
    "  exposure for global EV.",
